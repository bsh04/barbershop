...
class AuthService {
  ...
  Future<ResponseModel<IValueModel>> signInWithGoogle() async {
    ...
    if (!userSnapshot.exists) {
      _usersRef.doc(login).set({'name': cred.user.displayName, 'login': login, 'balance': 11200.0});
    }
  ...
  Future<ResponseModel<IValueModel>> signInWithVk() async {
    ...
    if (res.isValue) {
      ...
      if (data.isCanceled) {
        ...
        if (!userSnapshot.exists) {
          _usersRef.doc(login).set({'name': name, 'login': login, 'balance': 11200.0});
        }
